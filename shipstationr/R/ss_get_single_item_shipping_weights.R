...
ss_get_single_item_shipping_weights <- function(start_date = Sys.Date() - 7, end_date = Sys.Date() - 1) {
  ...
  # TODO: add 'package_type' and 'dimensions'
  
  # get total weights (lbs) for all recent shipments
  shipments <- shipstationr::ss_get_shipments(start_date = start_date, end_date = end_date) %>%
    dplyr::select(orderNumber, weight, shipmentCost) %>%
    tidyr::unnest() %>%
    dplyr::mutate(
    ...
    dplyr::select(orderNumber, ship_weight = value, ship_cost = shipmentCost)
  ...
  ss_get_orders(start_date = start_date - 1, end_date = end_date + 1) %>%
    ...
    dplyr::select(orderNumber, sku, skuvault_weight = value) %>%
